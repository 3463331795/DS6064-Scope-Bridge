...
- 安全 JSON CLI：`list`、`idn`、`health`、`probe-open`、`summary`、`freq`、`period`、`duty`、`vpp`、`capture`、`capture-multi`、`snapshot`、`latest`、`diagnose-channel`、`analyze-pwm`。
...
`snapshot` 是推荐的 AI 交接入口：它在一次 USB-TMC 会话中读取设备信息、每个通道的内置测量值，并生成合并 CSV、叠加 PNG 和 manifest。只需要波形文件时继续用 `capture-multi`。
